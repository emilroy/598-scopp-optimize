...
class Baseline:
    def __init__(self, environment, number_of_robots=None, plot=False, plot_settings=SCoPP_settings.plots()):
        # Initialize environment parameters
        if len(environment.starting_position) > 1:
            geographical_starting_positions = environment.starting_position
        else:
        ...
        self.robot_initial_positions_in_geographical = []
        origin = [0, 0]
        origin[1] = min(np.concatenate((np.transpose(geographical_starting_positions)[0], np.transpose(geographical_boundary_points)[0])))
        origin[0] = min(np.concatenate((np.transpose(geographical_starting_positions)[1], np.transpose(geographical_boundary_points)[1])))
        self.origin = origin

        if geographical_geo_fencing_zones:
            self.geographical_geo_fencing_zones_in_cartesian = [[] for item in range(len(geographical_geo_fencing_zones))]
            self.geographical_geo_fencing_zones_in_geographical = [[] for item in range(len(geographical_geo_fencing_zones))]
        else:
            self.geographical_geo_fencing_zones_in_cartesian = None
            self.geographical_geo_fencing_zones_in_geographical = None
        ...
        # Initialize variables for plotting
        self.rough_partitioning_x = [[] for _ in range(self.number_of_partitions)]
        self.rough_partitioning_y = [[] for _ in range(self.number_of_partitions)]
        self.final_partitioning_x = [[] for _ in range(self.number_of_partitions)]
        self.final_partitioning_y = [[] for _ in range(self.number_of_partitions)]
        self.cluster_centers = None
        self.conflict_cells_x = []
        self.conflict_cells_y = []
    ...
    def run(self, info=False):
        ...
        for robot, position in enumerate(self.robot_initial_positions_in_cartesian):
            ...
            initial_travel_distance[robot] += sqrt((partitions[robot][0][0] - position[0]) ** 2 + (partitions[robot][0][1] - position[1]) ** 2)
        ...
        for robot, partition in enumerate(partitions):
            ...
            self.rough_partitioning_x[robot] = [p[0] for p in partition]
            self.rough_partitioning_y[robot] = [p[1] for p in partition]
            self.final_partitioning_x[robot] = self.rough_partitioning_x[robot]
            self.final_partitioning_y[robot] = self.rough_partitioning_y[robot]
        ...
        self.data_distance_travelled_per_robot = np.array(sweep_lengths) * self.cell_size + np.array(initial_travel_distance)
        ...
        if info:
            ...
            print("\nStatistics:")
            print(" Tasks per robot:", self.data_tasks_per_robot)
            print(" Distance for each robot to travel:", self.data_distance_travelled_per_robot, "meters")
            print(" Mission completion time per robot:", self.data_completion_time_per_robot, "seconds")
            print(" Mission completion time of the swarm:", self.data_total_mission_completion_time, "seconds")

            print("\nArea allocation information:")
            print(" Cell width:", self.cell_size, "meters")
            print(" Cell area:", self.cell_area, "square meters")
            print(" Total area to be surveyed:", self.total_survey_area, "square meters")
            self.area_per_robot = np.array(self.data_tasks_per_robot) * self.cell_area
        ...
        return self.data_completion_time_per_robot, self.data_total_mission_completion_time
    ...
    def plot_partitioning(self):
        ...
        for robot_id in range(self.number_of_partitions):
            plt.scatter(self.rough_partitioning_x[robot_id], self.rough_partitioning_y[robot_id], marker="s",
                        ...
                        c=np.ones((len(self.rough_partitioning_x[robot_id]), 3)) * self.partition_colors[robot_id])
        plt.scatter(self.cluster_centers[0], self.cluster_centers[1], s=self.plot_robot_size, c='black')
        plt.title("Initial Partitioning")
        plt.axis("equal")
        ...
        plt.scatter(np.transpose(self.robot_initial_positions_in_cartesian)[0],
                    np.transpose(self.robot_initial_positions_in_cartesian)[1],
                    s=self.plot_robot_size, c="black")
        plt.title("Final Partitioning with Robot Positions")
        plt.axis("equal")
